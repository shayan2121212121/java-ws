...
public class FamilyTravel {
    ...
    public static int shortest(String[] edges){
        ...
        //Step 2: dist array for min weightwith initial max value
        int[][] costArr = new int[edges.length][53];
        for (int[] row : costArr) Arrays.fill(row, Integer.MAX_VALUE);
        costArr[0][52] = 0; // Start at city 0 with "infinite" last interval
        ...
        while(!pq.isEmpty()){
            ...
            for(int[] neighbor: neighbors){
                ...
                if(currNode.time+nextCost<costArr[nextCity][nextCost]){
                        if(nextCost<=currNode.interval){//interval check for non root nodes
                            costArr[nextCity][nextCost]=currNode.time+nextCost;
                            pq.offer(new Node(currNode.city, nextCity, costArr[nextCity][nextCost],nextCost));
                        }
